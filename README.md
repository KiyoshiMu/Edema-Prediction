...
We separate these samples into two groups by whether the therapy is effective. T-Test was conducted to show which features are significantly different between two groups. We selected features inside by condition that the feature belong to the 9 types above and is with p-value less than 0.1. If multiple features fit the condition but belong the same type, only the one with the least p-value is selected, since features in the same type are different in their filers and matrixes which only influence their emphasis but represent the same idiosyncrasy of images. Finally, 6 features, as following *Table 1*, were selected. These feature might imply whether the patient fit this therapy. Also, they could provide information about where should be studied in patients' brain to better understand the mechanism of the illness and the therapy.
